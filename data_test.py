...
# root = "/home/kun/Downloads/Pointnet_Pointnet2_pytorch-master/data/ModelNet"
# fns = []
# split = 'train'
#
#
...
# import cv2
#
# img = cv2.imread("/home/kun/Pictures/dancing.jpg")
# cv2.imshow("a", img)
# cv2.waitKey(0)
...
#
#     out = find_k_neighbor(ps, P, 2).numpy()
#     target = np.array([[[1, 2],
#                         [2, 1]]])
#
#     self.assertTrue(np.array_equal(target, out))
#     return np.array_equal(target, out)
...
class xconv(nn.Module):
    ...
    def forward(self, pts, fts):
        ...
        feat = feat.permute(0,2,3,1)                # (B, P, C_delta + C_in, K)
        feat = torch.matmul(feat, X).permute(0,3,1,2)  # (B, K, P, C_delta + C_in)
...
class PointCNN_cls(nn.Module):
    def __init__(self, num_class):
        ...
        C_out = 16*3
        C_delta = C_out // 2
        self.x_conv1 = xconv(0, C_delta, C_out, P=-1, K=8)

        # X_conv2
        C_in = C_out
        C_out = 32*3
        C_delta = C_in // 4
        self.x_conv2 = xconv(C_in, C_delta, C_out, P=384, K=12, D=2)

        # X_conv3
        C_in = C_out
        C_out = 64*3
        C_delta = C_in // 4
        self.x_conv3 = xconv(C_in, C_delta, C_out, P=128, K=16, D=2)

        # X_conv4
        C_in = C_out
        C_out = 128*3
        C_delta = C_in // 4
        self.x_conv4 = xconv(C_in, C_delta, C_out, P=-1, K=16, D=3)

        self.fc = nn.Sequential(
            nn.Conv1d(C_out, 64 * 3),
            nn.ELU(inplace=True),
            nn.BatchNorm1d(64 * 3),
            nn.Conv1d(64 * 3, num_class),
            nn.ELU(inplace=True),
